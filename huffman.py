...
class HuffmanTree:
	...
	def add_node(self, node_to_add, path_int: int, n: int) -> None:
		node = self.root
		...
		for _ in range(n):
			...
			node = child
		# Just print the debug stuff...
		node.value = node_to_add.value
		#print("We added "+str(node.value)+" to path: "+str("".join([str(int(x)) for x in debug_thing])))
